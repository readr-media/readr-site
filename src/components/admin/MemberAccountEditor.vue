  ...
  <div class="member-editor" :class="{ result: message, confirm: action === 'delete' || action === 'customEditor_cancel' || action === 'customEditor_set' }">
    <div class="member-editor__form" v-if="action === 'customEditor_exceedError'">
      <div class="error">
        <div class="error__container">
          ...
          <p v-text="$t('login.WORDING_ADMIN_EXCEED_ERROR_CUSTOMEDITOR')"></p>
        </div>
      </div>
    </div>
    <div class="member-editor__form" v-else-if="action === 'customEditor_setError'">
      <div class="error">
        <div class="error__container">
          <img src="/public/icons/exclamation.png" alt="error">
          <p v-text="$t('login.WORDING_ADMIN_SET_ERROR_CUSTOMEDITOR')"></p>
        </div>
      </div>
    </div>
    <div class="member-editor__form" v-else-if="action !== 'delete' && action !== 'customEditor_cancel' && action !== 'customEditor_set'">
      ...
      <div class="email">
        <span class="label" v-text="$t('login.WORDING_ADMIN_MEMBER_EDITOR_EMAIL') + '：'"></span>
        <div>
      ...
      <div class="role">
        <span class="label" v-text="$t('login.WORDING_ADMIN_ROLE') + '：'"></span>
        <div class="options">
          <Radio class="admin"
            v-for="role in roles"
            v-if="role.key > 1"
      ...
      <div class="btn--save" @click="save" v-if="!message">
        <span v-text="$t('login.WORDING_ADMIN_MEMBER_EDITOR_SAVE')" v-if="!shouldShowSpinner"></span>
    ...
    <div class="member-editor__form" v-else>
      ...
      <template v-for="(m, k) in members">
        <div class="nickname" :class="{ multiple: k > 0 }">
          <span class="label" v-text="$t('login.WORDING_ADMIN_MEMBER_EDITOR_NICKNAME') + '：'"></span>
      ...
      <div class="btn--set" v-if="!message">
        <div class="btn--set__confirm" @click="save"><span v-text="$t('login.WORDING_ADMIN_YES')"></span></div>
        <div class="btn--set__cancel" @click="closeEditor"><span v-text="$('login.WORDING_ADMIN_CANCEL')"></span></div>
      </div>
      <div class="message" v-else v-text="message"></div>
    </div>
  </div>
          ...
          const callback = ({ status }) => {
            ...
            if (status === 200) {
              if (this.action === 'delete') {
                this.message = this.$t('login.WORDING_ADMIN_MEMBER_EDITOR_DELETE_SUCCESSFUL') + '！'
              } else {
                this.message = this.title + this.$t('login.WORDING_ADMIN_SUCCESS') + '！'
              }
              this.$emit('updated')
            } else {
              debug('status', status)
              this.message = this.title + this.$t('login.WORDING_ADMIN_INFAIL')
            }
          }
